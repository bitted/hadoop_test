...
public class HBasicOperation {
	...
	public static HTable addFamily(Configuration conf, HBaseAdmin admin) throws Exception {
		...
		if (!tableDescriptor.hasFamily("fam2".getBytes())) {
			if (admin.isTableEnabled(tab)) {
				admin.disableTable(tab);
			}
			admin.addColumn(tab.getBytes(), new HColumnDescriptor("fam2"));
		}
		// }
		HTable table = new HTable(conf, tab);
		return table;
	...
	public static void put(HTable table) throws Exception {
		...
		for (int i = 0; i < 10; i++) {
			putRow1.add(("fam1").getBytes(), ("col" + i).getBytes(), ("value" + i).getBytes());
			table.put(putRow1);
		}
